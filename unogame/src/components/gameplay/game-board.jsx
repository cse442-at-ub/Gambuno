...
import { UnoCard, WildCard, CardBack, ColorPicker } from "./cards"
import { createDeck, dealCards, canPlayCard, applyCardEffect } from "../../lib/game-logic"
import { motion, AnimatePresence } from "framer-motion"

export function GameBoard({ numPlayers = 4 }) {
                    ...
                    <div className="relative">
                        ...
                        {(gameState.visibleDiscardPile || [gameState.lastCard]).map((card, index, array) => {
                            ...
                            return (
                                ...
                                >
                                    {card.type === "special" ? (
                                        <WildCard className="w-24 h-36 sm:w-28 sm:h-40 md:w-28 md:h-40 lg:w-28 lg:h-40 opacity-100" onClick={() => {}} disabled={false} />                                    ) : (
                                       <UnoCard
                                            color={card.color}
                                            number={card.value}
