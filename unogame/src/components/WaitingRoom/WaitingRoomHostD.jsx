...
const WaitingRoom = () => {
  ...
  useEffect(() => {
    if (gameID) {
      fetch(`https://se-dev.cse.buffalo.edu/CSE442/2025-Spring/cse-442c/api/HTTP.php?action=getPlayers&gameID=${gameID}`)
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-b from-orange-500 to-yellow-500 flex flex-col items-center justify-center relative px-6 overflow-hidden">
      {/* Back Button */}
      <button className="absolute top-4 left-4 p-2" aria-label="Back" onClick={() => navigate("/join-game-menu")}>
            ...
            players.map((player, index) => (
              <div key={index} className="flex justify-between items-center bg-white px-4 py-3 shadow-lg rounded-xl border-4 border-black">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-lg font-bold text-black">{player.playerID}</p>
                </div>
                <span className={`px-4 py-2 text-white font-bold rounded-lg ${player.skipped ? "bg-red-600" : "bg-green-500"}`}>
                  {player.skipped ? "Waiting" : "Ready"}
                </span>
              </div>
            ))
          )}
