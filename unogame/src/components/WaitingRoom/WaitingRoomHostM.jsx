...
const WaitingRoom = () => {
  ...
  // Retrieve gameID from local storage
  const gameID = localStorage.getItem("gameID");

  useEffect(() => {
    const fetchLobbyData = async () => {
      try {
        // Fetch the real-time lobby data from the database
        const response = await fetch(`/api/HTTP.php?gameID=${gameID}`);
  ...
  // In host version, always show the UI by providing a default empty player list.
  const playerList = lobby && lobby.playerList ? lobby.playerList : [];
            ...
            >
              ...
              <span
                className={`px-4 py-2 text-white font-bold rounded-lg ${
                  player.status === "Ready" ? "bg-green-500" : "bg-red-600"
                }`}
              >
                {player.status}
              </span>
